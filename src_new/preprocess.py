...
if __name__ == "__main__":
    ...
    df = df.dropna(subset=['fwrd_ret_1y'])
    ...
    df[rp_cols] = df.groupby('trading_day_et')[rp_cols].apply(lambda x: x.fillna(0)).reset_index(drop=True)
    ...
    def _get_last_three_years_yq(current_yq: List[int]) -> List[List[int]]:
    ...
    for year in range(year_start + 3, year_end + 1):
        for quarter in range(1, 5):
            ...
            last_three_years_yq = _get_last_three_years_yq([year, quarter])
            ...
            train_set.to_parquet(f'training_data/{year}_{quarter}.parquet', index=False)
            ...
            test_set.to_parquet(f'test_data/{year}_{quarter}.parquet', index=False)
